...
import { X, ExternalLink, Zap, RotateCcw } from 'lucide-react';
...
function ChordPill({ chord, transition, nextChord, isEndpoint }) {
  ...
  async function handleClick() {
    ...
    setPlaying(false);
    // Open explanation
    if (transition) setOpen(v => !v);
  }

  // Endpoint (first/last chord): just play on click
  if (isEndpoint) {
    return (
  ...
  return (
    <div className="relative flex flex-col items-center gap-1">
      {/* Pill: click = joue + ouvre explication */}
      <button
        onClick={handleClick}
        ...
        title="Cliquer pour jouer et voir l'explication"
      >
        {chord}
      </button>
      ...
      {/* Approach type label */}
      {transition && (
        <span className="text-[9px] text-jazz-muted leading-none text-center max-w-[72px] truncate">
          {transition.label}
        </span>
      )}
